         ...
         public:
            ...
            void addTween(TweenerParam& param) {
                ...
                for (int i =0 ; i < param.total_properties; i++ ) {
                 ...
                 std::cout<<" \nParamretro: props"<< (param).total_properties  << " time" << (param).time;
            ...
            void removeListener(TweenerListener *listener) {
                for (listenerIT = listeners.begin(); listenerIT != listeners.end(); ++listenerIT ) {
                    TweenerListener *tlistener = *listenerIT;
            ...
            void step(long currentMillis) {
                ...
                //for (int i=(tweens.size() - 1); i >= 0  ; i--,tweensIT--) {
                for (tweensIT = tweens.begin(); tweensIT != tweens.end(); ++tweensIT ) {
                        //std::cout<<" \nParamretro: props"<< (*param).total_properties  << " time" << (*param).time;
                        //std::cout<<" \nIT: started"<< (*tweensIT).started <<" props"<< (*tweensIT).total_properties  << " time" << (*tweensIT).time;
                            ...
                            if ((*tweensIT).timeCount < (*tweensIT).time) {
                                for (unsigned int i =0 ; i < (*tweensIT).total_properties; i++ ) {
                                   ...
                                   if (prop.ptrValue != NULL ) {
                                        ...
                                        *(prop.ptrValue) = res;
                                         //std::cout<<"\nRun equation("<<(*tweensIT).timeCount<<","<<prop.initialValue<<","<< prop.finalValue <<","<<(*tweensIT).time;
                                         //std::cout<<"\nValor parametro"<< *(prop.ptrValue);
                                   }
                                }
                           }
